...
\end{itemize}
\textbf{Importante}: Si un grafo G es conexo basta con ejecutar DFS una única vez (a nivel for porque todos serían vecinos indirectos del primer nodo). Si el grafo G NO es conexo, el DFS se ejecutará más de una vez (porque hay una componente aíslada que no está conectada con el resto del grafo).

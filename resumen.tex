...
Su implementación algorítmica es muy simple aunque este método es muy costoso dado que su complejidad temporal es $O(n^{3})$. \\
¿Por qué tal complejidad? Bueno... Porque se implementa con 3 for anidados. \\
...
\begin{itemize}
    \item Si necesitás todos los caminos más cortos entre todos los pares de nodos. 
    \item Cuando tenes pesos negativos pero no ciclos negativos (no es un requisito tener pesos negativos)
    \item Cuando el grafo NO es demasiado grande.
  
\end{itemize}
\subsubsection*{Orden de operaciones}
\begin{itemize}
    \item Se arma una matriz, donde la distancia de un nodo a sí mismo es 0. 
    ...
    \item En cada iteración, se hace un if que comprueba si la distancia de i a j es mayor que la distancia de i a k + la distancia de k a j. Si es asi, entonces se actualiza la distancia de i a j.
\end{itemize}
...
\begin{itemize}
    \item Estado: Son los valores de los parámetros que definen el subproblema. Normalmente, son los parámetros de la función que usamos al memorizar y resuelven el problema.
